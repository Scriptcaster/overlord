...
export class DocumentEditComponent implements OnInit {
  ...
  form = new FormGroup({
    documentType: new FormControl('Estimate'),
  });
  ...
  onSubmit() {
    ...
    // console.log(this.documentForm.value.generalRepair);
    this.documentForm.value.services = [
      {name: 'General Welding', value: this.documentForm.value.generalWelding},
      {name: 'generalRepair', value: this.documentForm.value.generalRepair}
    ];
  ...
  onAddThing() {
    (<FormArray>this.documentForm.get('things')).push(
      new FormGroup({
        aden: new FormControl(null, Validators.required),
      })
    );
  }
  ...
  private initForm() {
    ...
    if (this.editMode) {
      ...
      documentGeneralWelding = document.generalWelding;
      documentGeneralRepair = document.generalRepair;
      documentBasementDoor = document.basementDoor;

      documentFireEscapes = document.fireEscapes;
      documentAwnings = document.awnings;
      documentRailings = document.railings;

      documentFences = document.fences;
      documentStairs = document.stairs;
      documentGates = document.gates;

      documentSecurityDoor = document.securityDoor;
      documentWindowGuards = document.windowGuards;
      documentOtherServices = document.otherServices;
      ...
      if (document['things']) {
        for (let thing of document.things) {
          documentThings.push(
            new FormGroup({
              aden: new FormControl(thing.aden, Validators.required)
            })
          );
        }
      }
  ...
  generatePdf() {
    if (this.documentForm.value.generalRepair) { 
      var documentGeneralRepair: string = 'dot'; 
    } else { 
      var documentGeneralRepair: string = 'circle'; 
    }

    if (this.documentForm.value.generalWelding) { 
      var documentGeneralWelding: string = 'dot'; 
    } else { 
      var documentGeneralWelding: string = 'circle'; 
    }

    if (this.documentForm.value.basementDoor) { 
      var documentBasementDoor: string = 'dot'; 
    } else { 
      var documentBasementDoor: string = 'circle'; 
    }

    if (this.documentForm.value.fireEscapes) { 
      var documentFireEscapes: string = 'dot'; 
    } else { 
      var documentFireEscapes: string = 'circle'; 
    }

    if (this.documentForm.value.awnings) { 
      var documentAwnings: string = 'dot'; 
    } else { 
      var documentAwnings: string = 'circle'; 
    }

    if (this.documentForm.value.railings) { 
      var documentRailings: string = 'dot'; 
    } else { 
      var documentRailings: string = 'circle'; 
    }

    if (this.documentForm.value.fences) { 
      var documentFences: string = 'dot'; 
    } else { 
      var documentFences: string = 'circle'; 
    }

    if (this.documentForm.value.stairs) { 
      var documentStairs: string = 'dot'; 
    } else { 
      var documentStairs: string = 'circle'; 
    }

    if (this.documentForm.value.gates) { 
      var documentGates: string = 'dot'; 
    } else { 
      var documentGates: string = 'circle'; 
    }

    if (this.documentForm.value.securityDoor) { 
      var documentSecurityDoor: string = 'dot'; 
    } else { 
      var documentSecurityDoor: string = 'circle'; 
    }

    if (this.documentForm.value.windowGuards) { 
      var documentWindowGuards: string = 'dot'; 
    } else { 
      var documentWindowGuards: string = 'circle'; 
    }

    if (this.documentForm.value.otherServices) { 
      var documentOtherServices: string = 'dot'; 
    } else { 
      var documentOtherServices: string = 'circle';
    }
    ...
    if (this.documentForm.value.tax) {
      ...
      this.tax = '$' + (this.documentForm.value.price * 0.0625).toFixed(2); 
      this.totalPrice = '$' + formatNumber((this.documentForm.value.price + this.documentForm.value.price * 0.0625).toFixed(2));
      this.halfPrice = '$' + formatNumber(((this.documentForm.value.price+ this.documentForm.value.price * 0.0625).toFixed(2)/2));
    }
    ...
    this.estimate = {
      ...
      {fontSize: 10, alignment: 'left', margin: [0, 10, 0, 0], table: { widths: ['*', '*', '*', '*'], body: [
        ...
        [{ul: [
            {text: 'General Welding', listType: documentGeneralWelding},
            {text: 'General Repair', listType: documentGeneralRepair},
            {text: 'Basement Door', listType: documentBasementDoor}
          ]}, {ul: [
            {text: 'Fire Escapes', listType: documentFireEscapes},
            {text: 'Awnings', listType: documentAwnings},
            {text: 'Railings', listType: documentRailings}
          ]}, {ul: [
            {text: 'Fences', listType: documentFences},
            {text: 'Stairs', listType: documentStairs},
            {text: 'Gates', listType: documentGates}
          ]}, {ul: [
            {text: 'Security Door', listType: documentSecurityDoor},
            {text: 'Window Guards', listType: documentWindowGuards},
            {text: 'Other Services', listType: documentOtherServices}
      ]}]]}},
    ...
    switch(this.form.value.documentType) {
      case 'Estimate':
