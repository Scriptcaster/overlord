...
export class DocumentEditComponent implements OnInit {
    ...
    if(this.form.value.documentType === 'Estimate') {
      this.docDefinition = {
        ...
        { bold: true, alignment: 'right', margin: [390, 0, 0, -80], stack: [{alignment: 'center', table: { body: [
          ...
          [ this.recipeForm.value.date, this.recipeForm.value.number]
        ]}}]},
      ...
      console.log('works Estimate');
    } else if(this.form.value.documentType === 'Contract') {
      console.log('works Contract');
    } else if(this.form.value.documentType === 'Invoice') {
      console.log('works Invoice');
    }
  ...
  private initForm() {
    ...
    this.recipeForm = new FormGroup({
      number: new FormControl(recipeNumber, Validators.required),
